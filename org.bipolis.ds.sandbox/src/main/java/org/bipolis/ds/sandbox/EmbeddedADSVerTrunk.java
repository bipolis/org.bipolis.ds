...
public class EmbeddedADSVerTrunk {
  ...
  public static void main(String[] args) {
    try {
      ...
      // Create the server
      final EmbeddedADSVerTrunk ads = new EmbeddedADSVerTrunk(workDir);

      // Read an entry
      final Entry result = ads.service.getAdminSession().lookup(new Dn("dc=apache,dc=org"));
  ...
  private void addIndex(Partition partition, String... attrs) {
    ...
    final Set indexedAttributes = new HashSet();
  ...
  private void initDirectoryService(File workDir) throws Exception {
    ...
    systemPartition.setPartitionPath(
        new File(service.getInstanceLayout().getPartitionsDirectory(), systemPartition.getId())
        .toURI());
    ...
    final Partition fooPartition = addPartition("foo", "dc=department,dc=example,dc=com",
        service.getDnFactory());
    final Partition barPartition = addPartition("bar", "dc=bar,dc=com", service.getDnFactory());
    final Partition apachePartition = addPartition("apache", "dc=apache,dc=org",
        service.getDnFactory());
    ...
    // Inject the context entry for dc=foo,dc=com partition if it does not already
    // exist
    try {
      service.getAdminSession().lookup(fooPartition.getSuffixDn());
    } catch (final LdapException lnnfe) {
      final Dn dnFoo = new Dn("dc=department,dc=example,dc=com");
      final Entry entryFoo = service.newEntry(dnFoo);
    ...
    try {
      ...
      final Entry entryBar = service.newEntry(dnBar);
    ...
    if (!service.getAdminSession().exists(apachePartition.getSuffixDn())) {
      final Dn dnApache = new Dn("dc=Apache,dc=Org");
      final Entry entryApache = service.newEntry(dnApache);
   ...
   *
   */
  private void magic(DirectoryService service) throws Exception {


    // We are done !
    //
    final File ldifFile = Paths.get("/tmp/test.ldif").toFile();

    System.out.println(ldifFile.exists());
    final LdifFileLoader ldifFileLoader = new LdifFileLoader(service.getAdminSession(), ldifFile,
        null);
